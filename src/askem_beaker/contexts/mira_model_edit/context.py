...
class MiraModelEditContext(BaseContext):
    ...
    async def setup(self, context_info, parent_header):
        ...
        if isinstance(self.config["context_info"]["id"], str):
    ...
    async def send_mira_preview_message(
        ...
        try:
            preview = await self.evaluate(
                self.get_code("model_preview", {"var_name": self.var_name, "schema_name": self.schema_name}))
    ...
    async def merge_models(self, message):
        ...
        self.beaker_kernel.send_response(
            "iopub", "merge_models_response", content, parent_header=message.header
        )
        await self.send_mira_preview_message(parent_header=message.header)
